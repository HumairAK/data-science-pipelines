...
func TestAddContainerExecutorTemplate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			t.Log("executor configmap", os.Getenv("EXECUTOR_CABUNDLE_CONFIGMAP_NAME"))
			...
			// t.Logf("Volumes in the template: %#v", executorTemplate.Volumes)
			// t.Logf("VolumeMounts in the container: %#v", executorTemplate.Container.VolumeMounts)
			if executorTemplate.Volumes != nil {
				t.Logf("Volumes in the template: %#v", executorTemplate.Volumes)
			} else {
