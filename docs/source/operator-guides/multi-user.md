...
> Note: Kubeflow makes no hard security guarantees about Profile isolation.
> Note: User profiles have no additional isolation beyond what is provided by Kubernetes Namespaces.
...
> Note: Pipeline definitions are not isolated right now, and are shared across all namespaces, see [Current Limitations](#current-limitations) for more details.
