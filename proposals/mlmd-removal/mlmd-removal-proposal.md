...
- As a Data Scientist, I want to track my pipeline executions and their states without relying on external services, so
  I can have a simpler and more reliable system.
- As an ML Engineer, I want to store and retrieve input/output artifacts and their metadata directly through KFP API, so
  I can have better control and visibility over my pipeline data.
- As a KFP Admin, I want to reduce the number of services I need to maintain, so I can improve system
  reliability and reduce operational overhead.
- As a KFP Admin, I want to ensure that artifact endpoints are properly secured and authenticated, so I can
  maintain data privacy and prevent unauthorized access.
- As a Developer, I want to have a simplified architecture with fewer integration points, so I can more easily debug and
