...
def set_image_pull_secrets(
    ...
    if is_list_of_parameter_channel(secret_names):
        image_pull_secret = []
        for secret_name in secret_names:
            secret_name_parameter = parse_k8s_parameter_input(secret_name, task)
            image_pull_secret.append(pb.ImagePullSecret(secret_name_parameter=secret_name_parameter))
    elif is_list_of_strings(secret_names):
        image_pull_secret = [
